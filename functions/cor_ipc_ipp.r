cor_ipc_ipp <- function(orig_country,tax,df=df_final,period=2001:2019,year_ref=2019){
    ...
    cor[i] <- 1/ppp[ppp$orig_count==orig_country & ppp$year==as.character(period)[i],"price_lvl"]*ipc[ipc$country==orig_country & ipc$year==year_ref,"ipc"]/ipc[ipc$country==orig_country & ipc$year==as.character(period)[i],"ipc"]

####### With full datasets ###########
read_avia_par <- function(data,first_year=1900,freq_data="",unit="PAS",arr_dep=c("DEP","ARR")){
  oldw <- getOption("warn")
  options(warn = -1)
  # data="avia_par_uk.tsv"
...
#4 Passage toward list of matrices, with each a different data type (ex:Arrival of Passenger Carried)
  ...
  avia_par_country <- avia_par_country[avia_par_country$arr_dep %in% arr_dep & avia_par_country$unit==unit,]
...
read_avia_par_list <- function(files,freq_data="Q",unit ="PAS",arr_dep="DEP"){
  avia_par_PAS_DEP <- lapply(files, function(x) {
    read_avia_par(x,freq_data=freq_data,unit = unit,arr_dep=arr_dep)
  })
  ...
  PAS_DEP[PAS_DEP$arr_dep=="ARR",] <- PAS_DEP[PAS_DEP$arr_dep=="ARR",c(1:4,7:8,5:6,9:ncol(PAS_DEP))]
  return(PAS_DEP)
}
